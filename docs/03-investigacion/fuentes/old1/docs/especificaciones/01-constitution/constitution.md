...
- **Víctor**: dirige el negocio, consulta SANIA, toma decisiones, confirma los hechos físicos y prepara y envía los paquetes
...
- "referencia de unidad": identificador alfanumérico único de tres caracteres para cada unidad física; es interno y también visible como sufijo al final del título público del anuncio. El alfabeto, las colisiones, la reutilización y la identidad física de unidades idénticas sin etiqueta siguen pendientes
- "anuncio": publicación manual de Wallapop o Vinted que representa una única unidad física y cuyo título termina con su referencia de tres caracteres; el enlace puede conservarse de forma opcional, pero no es la clave necesaria para identificar la unidad
- "tarea de publicación": mensaje o acción de Telegram que, al hacerse ejecutable, identifica una unidad y una plataforma, con texto listo para copiar, imágenes bajo demanda y acciones para confirmar, aplazar o cancelar la sugerencia; la granularidad de generación por producto o por unidad en entradas múltiples sigue pendiente
...
- "ticket operativo": caso visible que SANIA no puede cerrar automáticamente cuando un flujo confirmado decide abrirlo; un correo dudoso también puede quedar simplemente sin aplicar porque T02-Q08 no fijó una regla universal de creación de tickets
- "línea de compra": parte conceptual de un pedido asociada a un producto o variante; LIVE no aportó correos AliExpress ni confirmó su identificador estable, campos de cantidad o total, que solo podrán usarse cuando aparezcan en plantillas reales validadas
...
- "situación física": condición material de una unidad separada del resultado económico; en el MVP se confirma si entra disponible o queda bloqueada por una incidencia, y los casos de devolución, sustitución o pérdida permanecen aplazados hasta disponer de evidencia real
...
- "unidad exacta": unidad física identificada por la referencia de tres caracteres incluida en el título del anuncio vendido y que debe reservarse desde el primer correo reconocido
- "unidad compatible": otra referencia disponible del mismo producto y variante; su uso como sustitución de la unidad exacta no tiene un caso real confirmado y exige conciliación
- "FIFO": criterio anterior para elegir la unidad compatible más antigua; no se aplica automáticamente en el modelo nuevo de un anuncio por unidad y queda pendiente de validar con un caso real
- "pendiente de conciliación": situación en la que SANIA no puede relacionar un hecho con seguridad y no inventa ni altera datos; su representación como ticket o como hecho sin aplicar depende de una regla todavía pendiente
...
- [en entrevista] **seguir pedidos de AliExpress** (`seguir-pedidos-aliexpress`): Primera versión: normalizar los correos de pedido y seguimiento y mantener pedidos, líneas y paquetes separados aunque AliExpress divida o consolide envíos. Los identificadores de línea, plantillas y datos que no estén probados quedan pendientes y no se completan por suposición.; necesita antes: aprobar-y-realizar-compras
- [en entrevista] **confirmar la recepción de pedidos** (`confirmar-recepcion-de-pedidos`): Primera versión: el seguimiento entregado deja el paquete pendiente de comprobación física. Telegram muestra el contenido esperado y ofrece Todo correcto o Abrir disputa/No OK; solo la comprobación física correcta autoriza crear stock. La granularidad por paquete o línea y la cadencia de recordatorios siguen pendientes.; necesita antes: seguir-pedidos-aliexpress

### Almacén

- [en entrevista] **dar entrada a las unidades recibidas** (`dar-entrada-al-almacen`): Primera versión: después de Todo correcto, clasificar cada producto como Stock para venta o Compra personal, crear una referencia alfanumérica de tres caracteres por unidad destinada a venta y lanzar inmediatamente sus tareas de publicación. Una recepción incorrecta no crea stock y queda como incidencia; costes, reembolsos y sustituciones detallados siguen pendientes.; necesita antes: confirmar-recepcion-de-pedidos
- [en entrevista] **controlar el stock y la trazabilidad** (`controlar-stock-y-trazabilidad`): Primera versión: conocer el estado y el lote de cada unidad física, su referencia visible de tres caracteres y sus anuncios y ventas; reservar atómicamente la unidad exacta indicada en el título y no permitir stock negativo. Si la referencia no puede conciliarse, no se aplica FIFO automáticamente: se abre una excepción.; necesita antes: dar-entrada-al-almacen
- [sin empezar] **ajustar existencias** (`ajustar-existencias`): Registrar correcciones justificadas cuando el recuento físico no coincida con SANIA, sin borrar el historial.; necesita antes: controlar-stock-y-trazabilidad

### Anuncios y ventas

- [en entrevista] **crear y mantener anuncios** (`crear-y-mantener-anuncios`): SANIA presenta secuencialmente el par Wallapop/Vinted de un objetivo de publicación antes de pasar al siguiente. Cada acción confirmable resuelve una unidad y una plataforma concretas, con título terminado en la referencia visible de tres caracteres, texto listo e imágenes bajo demanda; sigue pendiente decidir si la generación inicial se agrupa por producto o nace por unidad.; necesita antes: controlar-stock-y-trazabilidad
...
- [en entrevista] **comprobar qué productos están publicados** (`comprobar-productos-publicados`): Actividad bloqueada para el MVP: no se ha autorizado una lectura web o pública de Wallapop/Vinted y tampoco se definió cómo comunicar o acreditar una comprobación manual. SANIA solo muestra el estado interno originado por Anuncio creado y las tareas pendientes; la visibilidad real queda marcada como no verificable.; necesita antes: crear-y-mantener-anuncios
...
- [en entrevista] **registrar una venta confirmada** (`registrar-venta-confirmada`): Primera versión: desde el primer correo reconocido, extraer del título la referencia visible de tres caracteres y reservar exactamente esa unidad una sola vez. Una referencia ausente, alterada, ambigua o ya no disponible abre conciliación sin aplicar FIFO automáticamente ni inventar stock.; necesita antes: controlar-stock-y-trazabilidad
- [sin empezar] **analizar cuánto tardan los anuncios en venderse** (`analizar-rendimiento-de-anuncios`): Fase posterior: medir el tiempo de venta de cada anuncio y comparar tendencias por producto y plataforma usando la referencia visible solo para identificar la unidad, sin automatizar acciones dentro de Wallapop o Vinted. El instante inicial de la medición sigue pendiente.; necesita antes: crear-y-mantener-anuncios, registrar-venta-confirmada
- [en entrevista] **retirar anuncios cuando se agota el stock** (`retirar-anuncios-sin-stock`): Cuando el primer correo reserve una última unidad publicada en ambas plataformas, Telegram identifica la unidad y la otra plataforma y pide a Víctor retirar manualmente el anuncio alternativo. La prueba de retirada, el uso opcional del enlace y la cadencia de recordatorio siguen pendientes.; necesita antes: registrar-venta-confirmada, comprobar-productos-publicados
...
- [en entrevista] **registrar costes, ingresos y gastos** (`registrar-movimientos-economicos`): Primera versión: guardar el importe comunicado por la plataforma con su origen y mantenerlo provisional hasta el cierre verificable. El movimiento final de Vinted procede de TX-COMPLETE; la fuente del monedero de Wallapop, los costes completos y el pricing por márgenes siguen pendientes o en evolución.; necesita antes: dar-entrada-al-almacen, registrar-venta-confirmada
...
- [en entrevista] **atender alertas y confirmaciones** (`atender-alertas-y-confirmaciones`): Primera versión: Telegram muestra decisiones, tareas físicas y excepciones ligadas a su operación. Incluye las botoneras confirmadas de recepción y publicación; el texto libre sigue aportando contexto y las cadencias, silencios, respuestas contradictorias y segundas confirmaciones permanecen pendientes.
- [en entrevista] **resolver excepciones operativas** (`resolver-excepciones-operativas`): Mantener sin aplicar los hechos desconocidos, contradictorios o no conciliables y aplicar los bloqueos concretos confirmados, como una recepción No OK o el cierre pendiente de Wallapop. T02-Q08 y T08-Q04 dejan por decidir cuándo se crea un ticket y qué otras partes pueden continuar.; necesita antes: atender-alertas-y-confirmaciones
...
- **línea de compra de AliExpress**: identificador del pedido cuando sea observable e identificador de línea solo si una plantilla real demuestra que es estable, producto y variante únicamente cuando puedan demostrarse, cantidad comprada solo cuando esté presente en una plantilla validada, total final pagado solo cuando esté presente y se conozca su significado, correo de origen (origen: futuro corpus real de correos AliExpress; LIVE no aportó ninguna plantilla)
- **unidad de inventario**: referencia alfanumérica única de tres caracteres, pedido, paquete y línea de origen únicamente cuando sean identificables y demostrables, clasificación Stock para venta o Compra personal, situación física, coste de adquisición original, coste actual, estado disponible, publicada, reservada o afectada por incidencia, relación con tareas de publicación, anuncios y venta (origen: contenido físicamente confirmado mediante Todo correcto y clasificación confirmada del producto; la relación con pedido, paquete o línea se conserva solo cuando pueda demostrarse)
...
- **pedido y paquete de AliExpress**: identificadores externos, líneas incluidas, estado operativo, eventos originales de seguimiento, correos de origen (origen: correos transaccionales de AliExpress)
- **anuncio**: plataforma, URL o identificador opcional cuando se conserve por navegación o auditoría, producto y variante, referencia visible de tres caracteres de la unidad, título y descripción finales usados, estado independiente por plataforma, tarea y confirmación Anuncio creado que originaron el estado, fecha de publicación cuando se confirme, fecha de venta o retirada cuando procedió, tiempo hasta la venta y origen del momento inicial usado (origen: tarea de publicación ligada a unidad y plataforma y confirmación humana Anuncio creado; no depende de lectura web)
- **venta**: plataforma e identificadores externos, anuncio, título observado y referencia de tres caracteres extraída, producto y variante, referencia reservada, precio final provisional, número de transacción de Vinted cuando exista, evidencia de entrega separada de la evidencia de cierre, estado y fechas, correo de origen (origen: correos transaccionales de Wallapop y Vinted)
...
- **movimiento económico**: operación relacionada, tipo, importe, fecha, estado provisional o definitivo, origen (origen: compras, precios comunicados en correos, TX-COMPLETE de Vinted y, cuando se acuerde una fuente, movimientos del monedero Wallapop; gastos, reembolsos y correcciones requieren evidencia)
- **ticket operativo**: tipo y estado, prioridad, entidad relacionada, origen y descripción, historial, responsable, resolución y recordatorios (origen: incidencias y bloqueos para los que un flujo confirmado decide crear un ticket; la regla general para hechos dudosos sigue pendiente)
- **evento externo**: plataforma y remitente, identificadores del mensaje y del negocio, fecha y hora, datos extraídos, copia del contenido de origen, resultado aplicado, dejado sin aplicar o convertido en ticket cuando exista una regla confirmada (origen: correos transaccionales y respuestas estructuradas de Telegram)

## Integraciones

- **una cuenta de Gmail de solo lectura**: recibir y conservar los correos transaccionales de AliExpress, Wallapop, Vinted, Correos e InPost sin modificar el buzón
- **Telegram**: pedir comprobaciones físicas, entregar tareas de publicación, enviar imágenes bajo demanda, recoger botones ligados a cada operación y mantener visibles decisiones e incidencias sin cadencias inventadas
...
- **Q-9**: Todo título preparado para una unidad anunciada terminó con su referencia alfanumérica de tres caracteres y cada confirmación Anuncio creado quedó ligada a una única unidad y plataforma.
...
- Dar por resuelta la identidad física suponiendo que se colocarán o que se descartarán etiquetas QR o de código de barras; esa solución permanece abierta.
...
- Primera versión: leer perfiles, páginas o monederos de Wallapop o Vinted hasta que se autorice expresamente una fuente y se definan sus límites.
...
- ¿Qué correos o señales de Wallapop y Vinted permiten reconocer una cancelación y devolver la misma referencia al stock?
- ¿Cómo se representará una venta real con varias unidades o productos y cómo se repartirá su importe, aunque todo extra deba figurar antes en la descripción final del anuncio?
...
- ¿Qué alfabeto usa la referencia de tres caracteres, cómo se evitan colisiones, cuándo puede reutilizarse y qué ocurre al agotarse el espacio?
- ¿Cómo se mantiene la identidad exacta de varias unidades físicamente idénticas si no tienen etiqueta ni rasgo diferenciador?
- ¿Se elimina por completo la URL del anuncio o se conserva opcionalmente para navegación o auditoría, y dejan de usarse los parámetros b, i y r de Wallapop?
- ¿Las tareas de publicación se crean por cada unidad o existe alguna agrupación por producto cuando llegan varias unidades iguales?
...
- ¿Se autoriza alguna lectura web pública sin sesión o el MVP queda limitado por completo a Gmail y confirmaciones de Telegram?
...
- ¿Qué estados, prioridades, responsables y plazos tendrá un ticket operativo?
- ¿Qué plantillas reales de correo deben formar el conjunto mínimo de pruebas de cada plataforma y transportista?
...
- ¿Desde qué instante se medirá el tiempo de venta de un anuncio: la confirmación Anuncio creado, la publicación efectiva u otro hecho?
- [T10-Q03 / T10-Q04] ¿Qué datos son delicados, quién no debe verlos y qué pasaría si SANIA no estuviera disponible durante medio día?
- [T10-Q01 / T10-Q02] ¿Cuántos correos, pedidos, unidades, ventas y decisiones procesa hoy el negocio y qué ocurre si dos ventas intentan reservar a la vez la misma última unidad?
